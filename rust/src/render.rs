...
#[inline]
pub fn neon() -> Color {
    crate::theme::current().neon
}
#[inline]
pub fn cyan() -> Color {
    crate::theme::current().cyan
}
#[inline]
pub fn magenta() -> Color {
    crate::theme::current().magenta
}
#[inline]
pub fn amber() -> Color {
    crate::theme::current().amber
}
#[inline]
pub fn red() -> Color {
    crate::theme::current().red
}
#[inline]
pub fn dim() -> Color {
    crate::theme::current().dim
}
#[inline]
pub fn text() -> Color {
...
fn sparkline(values: &[f64], width: usize, hot: bool) -> Vec<Span<'static>> {
    if values.is_empty() {
        return vec![Span::styled(
            BLOCKS[0].to_string().repeat(width),
            Style::new().fg(dim()),
        )];
    }
    ...
    for &v in data {
        ...
        out.push(Span::styled(
            ...
            Style::new().fg(grade(frac, hot)),
        ));
    }
    if data.len() < width {
        out.push(Span::styled(
            BLOCKS[0].to_string().repeat(width - data.len()),
            Style::new().fg(dim()),
        ));
    }
    out
...
fn area_chart(
    ...
    grid.into_iter()
        .map(|row| {
            Line::from(
                row.into_iter()
                    .map(|(c, st)| Span::styled(c.to_string(), Style::new().fg(st)))
                    .collect::<Vec<_>>(),
            )
        })
        .collect()
...
pub fn cpu_panel(s: &Snapshot, hist: &[f64]) -> Paragraph<'static> {
    ...
    let mut usage = vec![
        Span::styled(format!("{:<9}", "usage"), Style::new().fg(dim())),
    ...
    let avg_freq = if c.freqs_mhz.is_empty() {
        0.0
    } else {
        c.freqs_mhz.iter().sum::<f64>() / c.freqs_mhz.len() as f64
    };
    ...
    for &u in &c.per_core {
        ...
        strip.push(Span::styled(
            BLOCKS[idx.min(BLOCKS.len() - 1)].to_string(),
            Style::new().fg(grade(u / 100.0, true)),
        ));
    }
...
pub fn gpu_panel(s: &Snapshot, hist: &[f64]) -> Paragraph<'static> {
    ...
    if !g.present {
        return panel(
            Text::styled(
                format!("no readable GPU ({})", g.vendor),
                Style::new().fg(dim()),
            ),
            "GPU",
            magenta(),
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{:<8}", temp),
            Style::new().fg(grade(g.temp_c.unwrap_or(0.0) / 90.0, true)),
        ),
    ...
    if let Some(total) = g.mem_total_mb {
        ...
        let mut m = vec![
            Span::styled(format!("{:<9}", "vram"), Style::new().fg(dim())),
            Span::styled(
                format!("{:.0}/{:.0} MB  ", used, total),
                Style::new().fg(cyan()),
            ),
        ];
...
pub fn mem_panel(s: &Snapshot) -> Paragraph<'static> {
    ...
    let mut lines: Vec<Line> = Vec::new();
    let mut r = vec![
        Span::styled(format!("{:<9}", "ram"), Style::new().fg(dim())),
    ...
    lines.push(kv(
        ...
        &format!(
            "{} / {}",
            human_bytes(m.used as f64),
            human_bytes(m.total as f64)
        ),
        text(),
    ));
    if m.swap_total > 0 {
        ...
        let mut sr = vec![
            Span::styled(format!("{:<9}", "swap"), Style::new().fg(dim())),
...
pub fn fan_panel(
    s: &Snapshot,
    hist: &std::collections::HashMap<String, Vec<f64>>,
) -> Paragraph<'static> {
    if s.fans.is_empty() {
        return panel(
            Text::styled("no ASUS fan channels", Style::new().fg(dim())),
            "FANS",
            blue(),
        );
    }
    ...
    for f in &s.fans {
        ...
        let mut row = vec![
            Span::styled(
                format!("{:<10}", f.label.replace('_', " ")),
                Style::new().fg(blue()),
            ),
            Span::styled(
                format!("{:>5} rpm  ", f.rpm),
                Style::new().fg(if f.rpm > 0 { neon() } else { dim() }),
            ),
        ];
...
pub fn battery_panel(cfg: &crate::config::Config, s: &Snapshot) -> Paragraph<'static> {
    ...
    if !b.present {
        return panel(
            Text::styled("no battery", Style::new().fg(dim())),
            "BATTERY",
            amber(),
        );
    }
    let pct = b.percent.unwrap_or(0.0);
    let pcol = if pct < 20.0 {
        red()
    } else if pct < 40.0 {
        amber()
    } else {
    ...
    let mut lines: Vec<Line> = Vec::new();
    let mut r = vec![
        Span::styled(format!("{:<9}", "charge"), Style::new().fg(dim())),
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            ...
            Style::new().fg(if b.ac_online == Some(true) {
                neon()
            } else {
                dim()
            }),
    ...
    if let Some(secs) = if charging {
        ...
        lines.push(Line::from(vec![
            Span::styled(
                format!("{:<9}", if charging { "to full" } else { "remaining" }),
                Style::new().fg(dim()),
            ),
            Span::styled(
...
pub fn storage_panel(s: &Snapshot) -> Paragraph<'static> {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{:<8}", nvme),
            Style::new().fg(grade(st.nvme_temp_c.unwrap_or(0.0) / 80.0, true)),
        ),
        Span::styled(format!("{:<9}", "root"), Style::new().fg(dim())),
        Span::styled(format!("{:.0}%", st.root_percent), Style::new().fg(cyan())),
    ]));
    let mut r = vec![
        Span::styled(format!("{:<9}", "disk"), Style::new().fg(dim())),
        Span::styled(
            format!(
                "{} / {}  ",
                human_bytes(st.root_used as f64),
                human_bytes(st.root_total as f64)
            ),
            Style::new().fg(text()),
        ),
    ];
...
pub fn profile_banner(s: &Snapshot) -> Paragraph<'static> {
    ...
    let body = Text::from(Line::styled(
        format!("{}  {}", icon, prof.to_uppercase()),
        Style::new().fg(color).add_modifier(Modifier::BOLD),
    ));
...
pub fn headline_strip(s: &Snapshot) -> Paragraph<'static> {
    ...
    tiles.push((
        c.temp_c
            ...
            .unwrap_or_else(|| "—".into()),
        "CPU TEMP",
        grade(c.temp_c.unwrap_or(0.0) / 95.0, true),
    ));
    tiles.push((
        format!("{:.0}%", c.overall),
        "CPU LOAD",
        grade(c.overall / 100.0, true),
    ));
    if g.present {
        tiles.push((
            g.temp_c
                ...
                .unwrap_or_else(|| "—".into()),
            "GPU TEMP",
            grade(g.temp_c.unwrap_or(0.0) / 90.0, true),
        ));
        tiles.push((
            g.util
                .map(|u| format!("{u:.0}%"))
                .unwrap_or_else(|| "—".into()),
            "GPU LOAD",
            grade(g.util.unwrap_or(0.0) / 100.0, true),
        ));
    }
    ...
    tiles.push((
        format!("{fmax}"),
        "FAN RPM",
        grade(fmax as f64 / FAN_RPM_MAX, true),
    ));
    let b = &s.battery;
    if b.present && b.rate_w.map(|r| r < 0.0).unwrap_or(false) {
        tiles.push((
            format!("{:.0}W", b.rate_w.unwrap().abs()),
            "SYS DRAW",
            amber(),
        ));
    ...
    let cell = 14usize;
    let vals: Vec<Span> = tiles
        .iter()
        .map(|(v, _, c)| {
            Span::styled(
                format!("{:^width$}", v, width = cell),
                Style::new().fg(*c).add_modifier(Modifier::BOLD),
            )
        })
        .collect();
    let labs: Vec<Span> = tiles
        .iter()
        .map(|(_, l, _)| {
            Span::styled(
                format!("{:^width$}", l, width = cell),
                Style::new().fg(dim()),
            )
        })
        .collect();
    panel(
        Text::from(vec![Line::from(vals), Line::from(labs)]),
        "SYSTEM AT A GLANCE",
        neon(),
    )
}
...
pub fn thermal_graph_panel(s: &Snapshot, cpu_hist: &[f64], gpu_hist: &[f64]) -> Paragraph<'static> {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            ct.map(|t| format!("   {t:.0}°C"))
                .unwrap_or_else(|| "   —".into()),
            Style::new().fg(grade(ct.unwrap_or(0.0) / 95.0, true)),
        ),
        Span::styled("      scale 0–100°C", Style::new().fg(dim())),
    ]));
    let cdata = if cpu_hist.is_empty() {
        vec![ct.unwrap_or(0.0)]
    } else {
        cpu_hist.to_vec()
    };
    ...
    if s.gpu.present {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                gt.map(|t| format!("   {t:.0}°C"))
                    .unwrap_or_else(|| "   —".into()),
                Style::new().fg(grade(gt.unwrap_or(0.0) / 90.0, true)),
            ),
            Span::styled("      scale 0–100°C", Style::new().fg(dim())),
        ]));
        let gdata = if gpu_hist.is_empty() {
            vec![gt.unwrap_or(0.0)]
        } else {
            gpu_hist.to_vec()
        };
...
pub fn fan_graph_panel(
    ...
    for (n, f) in s.fans.iter().enumerate() {
        ...
        lines.push(Line::from(vec![
            Span::styled(
                f.label.replace('_', " ").to_uppercase(),
                Style::new().fg(cyan()),
            ),
            Span::styled(
                format!("   {} ", f.rpm),
                Style::new().fg(if f.rpm > 0 { grade(frac, true) } else { dim() }),
            ),
            Span::styled("rpm", Style::new().fg(dim())),
            Span::styled(
                format!("      scale 0–{} rpm", FAN_RPM_MAX as i64),
                Style::new().fg(dim()),
            ),
        ]));
        let series = hist
            .get(&f.label)
            .cloned()
            .unwrap_or_else(|| vec![f.rpm as f64]);
...
pub fn fan_curve_panel(
    curves: &[crate::control::FanCurve],
    profile: &str,
    active: bool,
) -> Paragraph<'static> {
    ...
    if curves.is_empty() {
        return panel(
            Text::styled("no fan-curve data — needs asusd", Style::new().fg(dim())),
            &title,
            amber(),
        );
    }
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            if active {
                "  (active — drives the fans now)"
            } else {
                "  (edit only — not the live profile)"
            },
    ...
    for c in curves {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                if c.enabled {
                    ...
                    "curve disabled (firmware default)"
                },
        ...
        for p in &pcts {
            fl.push(Span::styled(
                format!("{p:>3} "),
                Style::new().fg(grade(*p as f64 / 100.0, true)),
            ));
        }
...
pub fn proc_detail_panel(d: Option<&crate::telemetry::ProcDetail>) -> Paragraph<'static> {
    let Some(d) = d else {
        return panel(
            Text::styled("select a process to inspect", Style::new().fg(dim())),
            "PROCESS DETAIL",
            cyan(),
        );
    };
    ...
    let status_col = if d.status == "run"
        || d.status == "sleep"
        || d.status == "running"
        || d.status == "sleeping"
    {
    ...
    let lines = vec![
        ...
        kv2(
            "ppid",
            d.ppid.map(|p| p.to_string()).unwrap_or_else(|| "—".into()),
            text(),
        ),
        kv2(
            "cpu",
            format!("{:.1}%", d.cpu),
            grade((d.cpu.min(100.0)) / 100.0, true),
        ),
...
pub fn gpu_proc_panel(s: &Snapshot) -> Paragraph<'static> {
    if !s.gpu.present {
        return panel(
            Text::styled("no GPU", Style::new().fg(dim())),
            "GPU PROCESSES",
            magenta(),
        );
    }
    if s.gpu_procs.is_empty() {
        return panel(
            Text::styled("no GPU compute processes", Style::new().fg(dim())),
            "GPU PROCESSES",
            magenta(),
        );
    }
    ...
    for p in &s.gpu_procs {
        lines.push(Line::from(vec![
            ...
            Span::styled(
                format!("{:<28}", p.name.chars().take(27).collect::<String>()),
                Style::new().fg(text()),
            ),
...
pub fn net_table(s: &Snapshot) -> Paragraph<'static> {
    ...
    let shown: Vec<_> = s
        .net
        .iter()
        .filter(|i| !i.is_virtual || i.up_bps + i.down_bps > 0.0)
        .take(10)
        .collect();
    for i in shown {
        ...
        lines.push(Line::from(vec![
            Span::styled(
                format!(
                    "{:<14}",
                    format!("{}{}", i.name, if i.is_virtual { " *" } else { "" })
                ),
                Style::new().fg(if i.is_virtual { dim() } else { cyan() }),
            ),
            Span::styled(
                format!("{:<6}", if i.is_up { "UP" } else { "down" }),
                Style::new().fg(if i.is_up { neon() } else { dim() }),
            ),
            Span::styled(
                format!("{:<16}", if i.ipv4.is_empty() { "—" } else { &i.ipv4 }),
                Style::new().fg(text()),
            ),
            Span::styled(
                format!("{:>11}", fmt_rate(i.down_bps).trim()),
                Style::new().fg(neon()),
            ),
            Span::styled(
                format!("{:>11}", fmt_rate(i.up_bps).trim()),
                Style::new().fg(amber()),
            ),
            Span::styled(
                format!("{:>9}", human_bytes(i.rx_total as f64)),
                Style::new().fg(dim()),
            ),
            Span::styled(
                format!("{:>9}", human_bytes(i.tx_total as f64)),
                Style::new().fg(dim()),
            ),
            Span::styled(
                format!("{:>9}", errs),
                Style::new().fg(if errs > 0 { red() } else { dim() }),
            ),
        ]));
    }
...
pub fn connections_panel(conns: &[crate::telemetry::NetConn]) -> Paragraph<'static> {
    if conns.is_empty() {
        return panel(
            Text::styled("no active inet connections", Style::new().fg(dim())),
            "CONNECTIONS",
            blue(),
        );
    }
    ...
    for c in conns.iter().take(16) {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                format!("{:<24}", cut(&c.laddr, 23)),
                Style::new().fg(cyan()),
            ),
            Span::styled(
                format!("{:<24}", cut(&c.raddr, 23)),
                Style::new().fg(text()),
            ),
    ...
    panel(
        Text::from(lines),
        &format!("CONNECTIONS  ({est} established, {lis} listening)"),
        blue(),
    )
}

pub fn bandwidth_graph_panel(
    down_hist: &[f64],
    up_hist: &[f64],
    cur_down: f64,
    cur_up: f64,
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            fmt_rate(cur_down).trim().to_string(),
            Style::new().fg(neon()),
        ),
        Span::styled(
            format!("      peak {}", fmt_rate(dmax).trim()),
            Style::new().fg(dim()),
        ),
    ]));
    let d = if down_hist.is_empty() {
        vec![0.0]
    } else {
        down_hist.to_vec()
    };
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            fmt_rate(cur_up).trim().to_string(),
            Style::new().fg(amber()),
        ),
        Span::styled(
            format!("      peak {}", fmt_rate(umax).trim()),
            Style::new().fg(dim()),
        ),
    ]));
    let u = if up_hist.is_empty() {
        vec![0.0]
    } else {
        up_hist.to_vec()
    };
